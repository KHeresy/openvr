...
void displayVR()
{
	...
	if (bFramesDecoded)
	{
		...
		{
			g_pOpenVRGL->Render([i](vr::Hmd_Eye eEye, glm::mat4 matModelView, glm::mat4 matProjection) {
...
initGLTexture(unsigned int nWidth, unsigned int nHeight)
{
	...
	g_pImageGL->BuildBall(5, 255, 255);
    return true;
}
                ...
                unsigned int &dispWidth, unsigned int &dispHeight)
{
    ...
    std::unique_ptr<VideoSource> apVideoSource(new VideoSource(video_file, apFrameQueue.get(), g_nBitDepth));
...
initCudaVideo()
{
    ...
    std::unique_ptr<VideoDecoder> apVideoDecoder(new VideoDecoder(videoFormat, g_oContext, g_eVideoCreateFlags, g_CtxLock));
    std::unique_ptr<VideoParser> apVideoParser(new VideoParser(apVideoDecoder.get(), g_pFrameQueue, &oFormatEx, &g_oContext));
